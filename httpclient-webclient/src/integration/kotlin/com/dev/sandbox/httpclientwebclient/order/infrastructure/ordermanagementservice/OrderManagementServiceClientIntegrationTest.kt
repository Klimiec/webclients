package com.dev.sandbox.httpclientwebclient.order.infrastructure.ordermanagementservice

import com.dev.example.httpclientwebclient.order.infrastructure.ordermanagementservice.OrderManagementServiceClient
import com.dev.example.httpclientwebclient.order.infrastructure.ordermanagementservice.OrderManagementServiceConnectionProperties
...
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceClientException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceIncorrectResponseBodyException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceReadTimeoutException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceRedirectionException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceRequestValidationException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceResourceNotFoundException
import com.dev.example.httpclientwebclient.order.infrastructure.util.ExternalServiceServerException
import com.dev.example.httpclientwebclient.order.infrastructure.util.SERVICE_NAME
...
import com.dev.sandbox.httpclientwebclient.order.infrastructure.ordermanagementservice.stub.internal.OrderManagementServiceContractFixture.ordersPlacedByPolishCustomer
import com.dev.sandbox.httpclientwebclient.order.infrastructure.ordermanagementservice.stub.internal.OrderManagementServiceStubBuilder
...
class OrderManagementServiceClientIntegrationTest : BaseIntegrationTest() {
    ...
    fun `verify custom tags for order-management-service (metrics)`(): Unit = runBlocking {
        ...
        // then
        meterRegistry.get("http.client.requests").tags(SERVICE_NAME, properties.clientName).timer()
    ...
    companion object {
        ...
        fun redirectErrors(): Stream<Arguments> {
            return Stream.of(
                Arguments.of(HttpStatus.MOVED_PERMANENTLY.value(), null),
