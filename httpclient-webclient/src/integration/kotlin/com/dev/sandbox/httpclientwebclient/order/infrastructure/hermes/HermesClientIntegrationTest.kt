...
import com.dev.example.httpclientwebclient.order.infrastructure.hermes.HermesClient
import com.dev.example.httpclientwebclient.order.infrastructure.hermes.HermesConnectionProperties
...
import com.dev.sandbox.httpclientwebclient.order.infrastructure.hermes.stub.HermesFixture
...
class HermesClientIntegrationTest : BaseIntegrationTest() {
    ...
    fun `when receive response with 4xx status code then throw exception`(
        ...
        // given
        val invoiceCreatedEvent = HermesFixture.invoiceCreatedEvent()
        stubs.hermes().willRejectAcceptInvoiceCreatedEventWith(status = statusCode, body = responseBody)
