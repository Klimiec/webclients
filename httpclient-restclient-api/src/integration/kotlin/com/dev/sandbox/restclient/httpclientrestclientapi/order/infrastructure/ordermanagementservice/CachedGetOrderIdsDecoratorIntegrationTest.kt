...
internal class CachedGetOrderIdsDecoratorIntegrationTest : BaseIntegrationTest() {
    ...
    fun setUp() {
        cache =
            (cachedOrderManagementServiceAdapter as CachedGetOrderIdsDecorator).getCache()
    }

    @Test
    fun `should not call external service when value is already present in the cache`() {
        // given
    ...
    @Test
    fun `should evict cache when size limit of the cache is exceeded`() {
        // given
        val clientId = anyClientId()
        stub.orderManagementService().willReturnOrdersFor(clientId, response = ordersPlacedBySomeCustomer())
        cachedOrderManagementServiceAdapter.getOrderIdsFor(clientId)
    ...
    @Test
    fun `should remove entry from cache after threshold period is passed since the last write`() {
        // given
    ...
    @Test
    fun `verify cache metrics contain cache name as a tag`() {
        // given
        val clientId = anyClientId()
        stub.orderManagementService().willReturnOrdersFor(clientId, response = ordersPlacedBySomeCustomer())
